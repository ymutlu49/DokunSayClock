...
export default function App(){
  ...
  /* Çizim */
  useEffect(function(){var cv=drawRef.current;if(!cv||!cvRef.current)return;var ctx=cv.getContext("2d");cv.width=cvRef.current.clientWidth*2;cv.height=cvRef.current.clientHeight*2;cv.style.width=cvRef.current.clientWidth+"px";cv.style.height=cvRef.current.clientHeight+"px";ctx.scale(2,2);ctx.lineCap="round";ctx.lineJoin="round";strokes.forEach(function(s){ctx.beginPath();ctx.globalCompositeOperation=s.type==="eraser"?"destination-out":"source-over";ctx.globalAlpha=s.type==="highlighter"?0.3:1;ctx.strokeStyle=s.type==="eraser"?"rgba(0,0,0,1)":s.color;ctx.lineWidth=s.width*2;s.points.forEach(function(p,i){if(i===0)ctx.moveTo(p.x*2,p.y*2);else ctx.lineTo(p.x*2,p.y*2);});ctx.stroke();ctx.globalAlpha=1;ctx.globalCompositeOperation="source-over";});},[strokes]);
  ...
  function drawMove(e){if(!drawing)return;var r=cvRef.current.getBoundingClientRect();var p={x:(e.clientX-r.left)/zoom,y:(e.clientY-r.top)/zoom};curStroke.current.push(p);var cv=drawRef.current;if(!cv)return;var ctx=cv.getContext("2d");ctx.globalCompositeOperation=tool==="eraser"?"destination-out":"source-over";ctx.globalAlpha=tool==="highlighter"?0.3:1;ctx.beginPath();ctx.strokeStyle=tool==="eraser"?"rgba(0,0,0,1)":penColor;ctx.lineWidth=(tool==="eraser"?24:tool==="highlighter"?penWidth*5:penWidth)*2;ctx.lineCap="round";var pts=curStroke.current;if(pts.length>=2){ctx.moveTo(pts[pts.length-2].x*2,pts[pts.length-2].y*2);ctx.lineTo(p.x*2,p.y*2);ctx.stroke();}ctx.globalAlpha=1;ctx.globalCompositeOperation="source-over";}
    ...
    <div style={{display:"flex",flexDirection:"column",height:"100vh",overflow:"hidden",fontFamily:"'Nunito','Segoe UI',system-ui,sans-serif"}}>
      ...
      <div style={{display:"flex",flex:1,overflow:"hidden"}}>
        ...
        <div ref={cvRef} style={Object.assign({flex:1,position:"relative",overflow:"auto"},cvBg)}>
          ...
          <canvas ref={drawRef} style={{position:"absolute",inset:0,zIndex:tool!=="select"?25:0,pointerEvents:tool!=="select"?"auto":"none",cursor:tool==="pen"||tool==="highlighter"?"crosshair":tool==="eraser"?"cell":tool==="text"?"text":"default"}} onPointerDown={drawStart} onPointerMove={drawMove} onPointerUp={drawEnd} onPointerCancel={drawEnd} onClick={handleCanvasClick}/>
          {/* Zoom wrapper */}
